...
class TrajectoryDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Q-Drop keeps the image layout and complete tool history but removes
        # the natural-language task.  The normal stream still produces the
        # recurrent latents; this second stream tests whether the action can
        # be recovered from those latents when the direct question shortcut
        # is unavailable.
        clean_q = clean_question(last["question"])
        image_tag_count = clean_q.count("<image>")
        qdrop_question = (
            ("<image>\n" * image_tag_count)
            + "[Task question masked. Execute the next action from visual "
              "evidence, interaction history, and the recurrent latent state.]"
        )
        ...
        qdrop_sequence, qdrop_attention, qdrop_layout = self._align_action_spans(
            rows,
        ...
        ess_encoded = []
        qdrop_prefix_ends = []
        qdrop_target_span_starts = []
        qdrop_target_span_ends = []
        qdrop_block_ends = []
        for row, (starts, ends, prefix_end), qd_layout in zip(
            rows, layout, qdrop_layout
        ):
            ...
            qd_starts, qd_ends, qd_prefix_end = qd_layout
            qdrop_prefix_ends.append(qd_prefix_end)
            qdrop_target_span_starts.append(qd_starts)
            qdrop_target_span_ends.append(qd_ends)
            qdrop_block_ends.append(qd_ends[-1])
        ...
        result = {
            ...
            "qdrop_attention_mask": torch.tensor(qdrop_attention, dtype=torch.long),
            ...
            "qdrop_target_span_starts": qdrop_target_span_starts,
            "qdrop_target_span_ends": qdrop_target_span_ends,
            "qdrop_block_ends": torch.tensor(qdrop_block_ends, dtype=torch.long),
...
class TrajectoryCoLTTrainer(CoLTTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        ess_enabled = self.ess_weight > 0.0
        ...
        qd_attention_mask = (
            inputs.get("qdrop_attention_mask") if qdrop_enabled else None
        )
        qd_prefix_ends = (
            inputs.get("qdrop_prefix_ends")[0] if qdrop_enabled else None
        )
        qd_span_starts = (
            inputs.get("qdrop_target_span_starts") if qdrop_enabled else None
        )
        qd_span_ends = (
            inputs.get("qdrop_target_span_ends") if qdrop_enabled else None
        )
        qd_block_ends = (
            inputs.get("qdrop_block_ends")[0] if qdrop_enabled else None
        )
        if qdrop_enabled:
            ...
            qd_cache = None
            # Start from 0 so system prompt, user prompt, and vision tokens enter qd_cache
            qd_cursor = 0
            qd_inserted = 0
        else:
        ...
        qdrop_per_kind = defaultdict(list)
        ...
        for boundary_index in range(boundary_count):
            ...
            expected_action_tokens = sum(
                end - start
                for start, end, enabled in zip(
                    span_starts, span_ends, span_active
                )
                if enabled
            )

            # Evaluate the masked-question action branch before building the
            # normal action and ESS graphs.  Detached latent leaves let us
            # free this second long transformer graph immediately; their
            # gradients are injected back into the live recurrent chain by a
            # zero-valued surrogate during the normal backward below.
            qdrop_loss = current.sum() * 0.0
            qdrop_latent_grads = None
            qdrop_backed = False
            qd_local_cache = None
            qd_cached_len = 0
            qd_action_seed_position = None
            if qdrop_enabled:
                qd_prefix_end = int(qd_prefix_ends[boundary_index].item())
                qd_target_end = int(qd_block_ends[boundary_index].item())
                qd_starts = [
                    int(value) for value in qd_span_starts[boundary_index]
                ]
                qd_ends = [
                    int(value) for value in qd_span_ends[boundary_index]
                ]
                if qd_cursor < qd_prefix_end:
                    qd_interval = qd_base_embeddings[:, qd_cursor:qd_prefix_end]
                    qd_interval_positions = (
                        qd_positions[..., qd_cursor:qd_prefix_end] + qd_inserted
                    )
                    with torch.no_grad():
                        qd_prefix_outputs = backbone(
                            ...
                            position_ids=qd_interval_positions,
                    ...
                    qd_cache = detach_kv_cache(
                        qd_prefix_outputs.past_key_values
                    )
                    qd_cursor = qd_prefix_end
                    del qd_prefix_outputs
                qd_local_cache = qd_cache
                qd_cached_len = qd_prefix_end + qd_inserted
                qd_prefix_last_position = (
                    qd_positions[..., qd_prefix_end - 1:qd_prefix_end]
                    + qd_inserted
                )
                qd_latent_leaves = [
                    latent.detach().requires_grad_(True) for latent in latents
                ]
                for stage, predicted_latent in enumerate(qd_latent_leaves):
                    qd_latent_outputs = backbone(
                    ...
                    qd_local_cache = qd_latent_outputs.past_key_values
                    qd_cached_len += 1
                    del qd_latent_outputs
                qd_action_seed_position = (
                    qd_prefix_last_position + modules.stages + 1
                )
                qd_suffix_ids = qd_input_ids[:, qd_prefix_end:qd_target_end]
                qd_suffix_embeddings = qd_base_embeddings[
                    :, qd_prefix_end:qd_target_end
                ]
                qd_action_inputs = torch.cat(
                    [qd_latent_leaves[-1].unsqueeze(1),
                     qd_suffix_embeddings[:, :-1]],
                    dim=1,
                )
                qd_inserted_after = qd_inserted + modules.stages + 1
                qd_action_positions = torch.cat([
                    qd_action_seed_position,
                    qd_positions[..., qd_prefix_end:qd_target_end - 1]
                    + qd_inserted_after,
                ], dim=-1)
                qd_action_outputs = backbone(
                ...
                qd_labels = torch.full_like(qd_suffix_ids, -100)
                qd_token_count = 0
                for start, end, enabled in zip(
                    qd_starts, qd_ends, span_active
                ):
                    if not enabled:
                        continue
                    qd_labels[:, start - qd_prefix_end:end - qd_prefix_end] = (
                        qd_suffix_ids[
                            :, start - qd_prefix_end:end - qd_prefix_end
                        ]
                    )
                    qd_token_count += end - start
                if qd_token_count != expected_action_tokens:
                    raise RuntimeError(
                        f"normal/QDrop action-token mismatch at "
                        f"{inputs['boundary_ids'][boundary_index]}: "
                        f"{expected_action_tokens} != {qd_token_count}"
                    )
                if qd_token_count:
                    qdrop_loss = chunked_token_ce(
                        qd_action_outputs.last_hidden_state,
                        qd_labels,
                        lm_head,
                        self.chunk_size,
                    )
                    ...
                    qdrop_loss = (
                        qd_action_outputs.last_hidden_state.sum() * 0.0
                    )
                qdrop_loss_values.append(qdrop_loss.detach())
                if stream_backward:
                    qdrop_weighted = (
                        qdrop_weighted + qdrop_loss.detach() * qd_token_count
                    )
                else:
                    qdrop_weighted = qdrop_weighted + qdrop_loss * qd_token_count
                if stream_backward:
                    qdrop_scale = (
                    ...
                    qdrop_latent_grads = [
                        (torch.zeros_like(leaf) if leaf.grad is None
                         else leaf.grad.detach().clone())
                        for leaf in qd_latent_leaves
                    ]
                    qdrop_backed = True
                    qdrop_loss = qdrop_loss.detach()
                    qd_local_cache = detach_kv_cache(qd_local_cache)
                    del qd_action_outputs, qd_action_inputs
                    del qd_action_positions, qd_labels
                    del qd_suffix_ids, qd_suffix_embeddings
                    del qd_latent_leaves
            ...
            if qdrop_enabled and qd_token_count:
                qdrop_per_kind[kind].append(qdrop_loss.detach())
                qdrop_per_tool[tool_label].append(qdrop_loss.detach())
            ...
            if ess_enabled:
                ...
                ess_result = {"loss": memory.sum() * 0.0}
            boundary_ess_active = ess_active[boundary_index].reshape(()).to(
                ess_result["loss"].dtype
            )
            if ess_enabled and bool(ess_active[boundary_index].item()):
                ess_losses.append(
            ...
            if stream_backward:
                ...
                if qdrop_latent_grads is not None:
                    ...
                    scaled_boundary_loss = scaled_boundary_loss + (
                        qdrop_surrogate - qdrop_surrogate.detach()
                    )
                backward_kwargs = (
            ...
            if boundary_index + 1 < boundary_count:
                ...
                if qdrop_enabled:
                    next_qd_prefix = int(qd_prefix_ends[boundary_index + 1].item())
                    if next_qd_prefix <= qd_prefix_end:
                        raise RuntimeError("next QDrop boundary did not advance")
                    ...
                    del qd_advance_outputs, qd_advance_inputs, qd_advance_positions
        ...
        qdrop_total = (
            qdrop_weighted / max(1, action_tokens)
        ...
        metrics = {
            ...
            "qdrop_effective_weight": torch.tensor(
                self.qdrop_action_weight, device=total.device
            ),
        ...
        if ess_enabled:
            metrics["ess_nll"] = ess_total.detach()
            metrics["ess_effective_weight"] = torch.tensor(
                ess_scale, device=total.device
            )
        ...
        for kind, values in qdrop_per_kind.items():
            metrics[f"qdrop_action_ce_{kind}"] = torch.stack(values).mean()
    ...
    def save_model(self, output_dir=None, _internal_call=False):
        ...
        torch.save({
            ...
            "qdrop_question": "masked before action; latent generated from full question",
        }, os.path.join(output_dir, "colt_modules.pt"))
...
def main():
    ...
    ap.add_argument("--qdrop-action-weight", type=float, default=0.0)
    ...
    ap.add_argument("--init-checkpoint", default="",
                    help="Load model/latent/ESS weights only and start a fresh optimizer at step 0.")
    ...
    if args.init_checkpoint:
        trainer._load_from_checkpoint(args.init_checkpoint, model=trainer.model)
