...
INIT=$SRC/checkpoints/ac_ess_nc_stop_v1/checkpoint-2500
OUT=$ROOT/checkpoints/qdrop_v1
...
test -s "$DATA"
test -s "$EVAL"
mkdir -p "$OUT" "$ROOT/logs" "$ROOT/reports"
...
exec "$TR" --standalone --nproc_per_node=8 "$ROOT/code/train_ess_v3_qdrop.py" \
  ...
  --action-weight 1 --ess-weight 0.3 --qdrop-action-weight 0.3 \
  --semantic-weight 0 --ess-stop-weight 0.1 --ess-warmup-steps 1 \
  --num-train-epochs 1 --max-steps -1 --save-steps 250 --eval-steps 250 \
  --logging-steps 1 --eval-decode-cases 20 --eval-decode-max-new-tokens 256 \
