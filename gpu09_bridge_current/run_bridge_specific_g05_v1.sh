...
OUT=${OUTPUT_DIR:-$ROOT/checkpoints/bridge_specific_g05_v1}
LOG=${LOG_FILE:-$ROOT/logs/bridge_specific_g05_v1.log}
...
"$PY" "$GUARD" --input "$EVAL"
exec "$TR" --standalone --nproc_per_node=8 "$ROOT/code/train_joint_semantic_route.py" \
  ...
  --action-weight 1.0 --qdrop-action-weight 0.0 \
