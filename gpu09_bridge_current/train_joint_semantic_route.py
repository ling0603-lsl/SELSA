    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        for boundary_index in range(boundary_count):
            ...
            if qdrop_enabled:
                ...
                # Pure Bridge Bottleneck: No latents (z1..z4) are written into Q-Drop KV cache,
                # and no z4 action seed is prepended. The Q-Drop action branch must decode ALL
                # executable semantics (control token, arguments, answer) purely from the Bridge tokens.
                ...
                qd_action_positions = torch.cat([
                    ...
                    + qd_inserted + semantic_token_count,
                ], dim=-1)
                ...
                # Labels: positions 0..semantic_token_count-2 get -100.
                # Position semantic_token_count-1 (terminal bridge token) predicts suffix_ids[:, 0]!
                qd_suffix_labels = (
                    qd_suffix_ids
                ...
                qd_labels = torch.cat([
                    torch.full(
                        (qd_suffix_ids.shape[0], semantic_token_count - 1), -100,
                ...
                if bool(action_active[boundary_index].item()):
                    with torch.no_grad():
                        ...
                        qd_branch_logits = lm_head(
                            qd_action_hidden[
                                :, semantic_token_count - 1:semantic_token_count
                            ].detach()
                        ).float().squeeze(1)
                ...
                if stream_backward and bool(action_active[boundary_index].item()):
                    qdrop_scale = self.qdrop_action_weight / max(
                    ...
                    qdrop_latent_grads = None  # No latents in pure bridge Q-Drop
                    ...
                    del qd_branch_logits, qd_branch_nll, qd_branch_target
            ...
            if stream_backward:
                ...
                boundary_loss = boundary_loss + (
                    self.qdrop_action_weight * qdrop_loss
                )
                if align_loss is not None:
                    boundary_loss = boundary_loss + (
